locals {
  output_attributes = {
    ...
    # Private Services Connection Attributes
    private_services_connection_status  = lookup(local.private_services_connection, "enable", true)
    private_services_peering_connection = lookup(local.private_services_connection, "enable", true) ? google_service_networking_connection.private_services_connection[0].network : null
    private_services_range_name         = lookup(local.private_services_connection, "enable", true) ? google_compute_global_address.private_services_range[0].name : null
    private_services_range_id           = lookup(local.private_services_connection, "enable", true) ? google_compute_global_address.private_services_range[0].id : null
    private_services_connection_id      = lookup(local.private_services_connection, "enable", true) ? google_service_networking_connection.private_services_connection[0].id : null
    private_services_range_address      = lookup(local.private_services_connection, "enable", true) ? google_compute_global_address.private_services_range[0].address : null
  }
  output_interfaces = {
  }
} 