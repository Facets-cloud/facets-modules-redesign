...
locals {
  ...
  # Private Services Connection configuration with defaults
  private_services_connection = var.instance.spec.private_services_connection != null ? var.instance.spec.private_services_connection : {
    enable        = true
  ...
  # Add a short random suffix to prevent naming conflicts
  random_suffix = substr(md5("${local.clean_name_prefix}-${var.instance.spec.vpc_cidr}"), 0, 6)
  # Truncate to ensure room for random suffix and longest resource suffix
  # Keep first 20 chars + 6 char random + longest suffix (~20 chars) = ~46 chars (well under 63 limit)
  base_name_prefix = length(local.clean_name_prefix) > 20 ? substr(local.clean_name_prefix, 0, 20) : local.clean_name_prefix
  name_prefix      = "${local.base_name_prefix}-${local.random_suffix}"
...
# Private Services Connection for managed services (Cloud SQL, etc.)
resource "google_compute_global_address" "private_services_range" {
  count = lookup(local.private_services_connection, "enable", true) ? 1 : 0

  name          = "${local.name_prefix}-private-services-range"
  purpose       = "VPC_PEERING"
  address_type  = "INTERNAL"
  prefix_length = lookup(local.private_services_connection, "prefix_length", 16)
  network       = google_compute_network.main.id

  # Use ip_cidr_range if provided, otherwise let Google auto-allocate
  address = lookup(local.private_services_connection, "ip_cidr_range", null) != null ? (
    split("/", lookup(local.private_services_connection, "ip_cidr_range", ""))[0]
  ) : null
