variable "instance" {
  ...
  validation {
    condition     = contains(["2.6.3", "2.7.2", "2.8.1"], var.instance.spec.version_config.kafka_version)
    error_message = "Kafka version must be one of: 2.6.3, 2.7.2, 2.8.1"
  }

  validation {
    condition     = contains(["kafka.t3.small", "kafka.m5.large", "kafka.m5.xlarge", "kafka.m5.2xlarge"], var.instance.spec.version_config.instance_type)
    error_message = "Instance type must be one of: kafka.t3.small, kafka.m5.large, kafka.m5.xlarge, kafka.m5.2xlarge"
  }
