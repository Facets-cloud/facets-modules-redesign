...
**K8s Platform**
`Helm` `Ingress/Gateway Fabric` `cert-manager` `ConfigMap` `Secrets` `PVC (k8s_standard)` `Access Controls` `Callbacks` `K8s Resources` `Gateway API CRD` `VPA` `Artifactories` `Argo Stack (gcp)` `Service/Argo (argo_gcp)`
