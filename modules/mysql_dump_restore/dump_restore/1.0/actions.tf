locals {
  action_cronjobs = {
    ...
    dump-restore-k8s = {
      ...
      description = "Run dump-only restore for all configured databases. Source is read-only; target reset requires allow_target_reset."
      mode        = "run"
    }
    reset-target-db-k8s = {
      cronjob     = "${local.safe_name}-driver"
    ...
    verify-restore-k8s = {
      ...
      description = "Compare table counts and row counts between source and target."
      mode        = "verify"
    }
    show-status-k8s = {
...
resource "facets_tekton_action_kubernetes" "mysql_dump_restore" {
