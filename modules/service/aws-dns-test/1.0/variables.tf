...
variable "environment" {
  ...
  type = object({
    ...
    # Environment-wide default pod dnsConfig, set on the `environment`
    # configuration module as spec.default_dns_config. Must be declared here:
    # Terraform object types are closed, so an undeclared attribute is silently
    # stripped before the module sees it and the default would never arrive.
    default_dns_config = optional(any, {})
  })
  default = {
    name        = "default"
