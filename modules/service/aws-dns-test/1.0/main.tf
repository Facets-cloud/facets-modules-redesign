...
module "app-helm-chart" {
  ...
  # TEST PIN: points at facets-utility-modules#48 (feat-dns-config-support) so the
  # dnsConfig chain can be validated before that PR merges. Revert to the
  # unpinned source once #48 lands.
  source         = "github.com/Facets-cloud/facets-utility-modules//application/2.0?ref=feat-dns-config-support"
  namespace      = local.namespace
