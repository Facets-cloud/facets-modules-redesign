...
# Gateway API HTTP-01 ClusterIssuers for nginx-gateway-fabric and other Gateway API implementations
module "cluster-issuer-gateway-staging" {
  depends_on      = [helm_release.cert_manager]
  source          = "github.com/Facets-cloud/facets-utility-modules//any-k8s-resource"
  name            = "letsencrypt-staging-gateway-http01"
  namespace       = local.cert_mgr_namespace
  advanced_config = {}
  ...
  data = {
    ...
    spec = {
      acme = {
        ...
        privateKeySecretRef = {
          name = "letsencrypt-staging-gateway-http01-account-key"
        }
        solvers = [
          {
            http01 = {
...
module "cluster-issuer-gateway-prod" {
  depends_on      = [helm_release.cert_manager]
  source          = "github.com/Facets-cloud/facets-utility-modules//any-k8s-resource"
  name            = "letsencrypt-prod-gateway-http01"
  namespace       = local.cert_mgr_namespace
  advanced_config = {}
  ...
  data = {
    ...
    spec = {
      acme = {
        ...
        privateKeySecretRef = {
          name = "letsencrypt-prod-gateway-http01-account-key"
        }
        solvers = [
          {
            http01 = {
