variable "instance" {
  description = "K8ssandra Operator deployment configuration"
  type = object({
    kind    = string
    flavor  = string
    version = string
    spec = object({
      namespace = optional(string, "cassandra-system")
      resources = object({
        cpu_request    = string
        cpu_limit      = string
...
variable "instance_name" {
  description = "The architectural name for the K8ssandra Operator resource"
  type        = string
}
