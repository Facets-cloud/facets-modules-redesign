locals {
  action_cronjobs = {
    preflight-checks-k8s = {
      ...
      description = "Run read-only source and target checks without changing data."
      mode        = "preflight"
    }
    dump-restore-k8s = {
      cronjob     = "${local.safe_name}-driver"
      description = "Run dump-only restore for all configured Mongo databases. Source is read-only; target reset requires allow_target_reset."
      mode        = "run"
    }
    reset-target-db-k8s = {
      cronjob     = "${local.safe_name}-driver"
      description = "Drop only configured target databases, guarded by allow_target_reset."
      mode        = "reset-target"
    }
    verify-restore-k8s = {
      cronjob     = "${local.safe_name}-verify"
      description = "Compare collection and document counts between source and target."
      mode        = "verify"
    }
    show-status-k8s = {
    ...
    debug-runner-k8s = {
      cronjob     = "${local.safe_name}-driver"
      description = "Create a short-lived debug pod with the runner image and env."
      mode        = "debug-shell"
    }
  }
}

resource "facets_tekton_action_kubernetes" "mongo_dump_restore" {
  ...
  steps = [
    {
      ...
      script = <<-EOT
        ...
        mode_patch() {
          printf '[{"op":"add","path":"/spec/template/spec/containers/0/env/-","value":{"name":"ACTION_MODE_OVERRIDE","value":"%s"}}]' "$(json_escape "$ACTION_MODE")"
        }
        debug_patch() {
          printf '[{"op":"replace","path":"/spec/template/spec/containers/0/command","value":["bash","-lc"]},{"op":"replace","path":"/spec/template/spec/containers/0/args","value":["echo debug pod ready; sleep 3600"]},{"op":"add","path":"/spec/template/spec/activeDeadlineSeconds","value":3660}]'
        }
        ...
        if [ "$ACTION_MODE" = "debug-shell" ]; then
          ...
          echo "kubectl exec -n $NAMESPACE -it $POD -- bash"
          exit 0
        fi
