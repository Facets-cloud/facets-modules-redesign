locals {
  service_account_name = lookup(var.instance.spec, "service_account_name", "facets-admin")
...
resource "null_resource" "add_k8s_creds_backend" {
  triggers = {
    ...
    cluster_id = var.environment.id
  }

  provisioner "local-exec" {
    command = <<EOF
curl -X POST "https://${var.environment.cc_host}/cc/v1/clusters/${var.environment.id}/credentials" \
  -H "accept: */*" \
  ...
  -H "X-DEPLOYER-INTERNAL-AUTH-TOKEN: ${var.environment.cc_auth_token}"
EOF
  }
