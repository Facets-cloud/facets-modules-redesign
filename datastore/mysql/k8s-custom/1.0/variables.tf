variable "instance" {
  description = "Deploys and manages MySQL database instances on Kubernetes using Helm charts with secure defaults and high availability"
  type = object({
    kind    = string
    flavor  = string
    version = string
    spec = object({
      version_config = object({
        mysql_version = optional(string, "8.4")
      ...
      restore_config = optional(object({
        ...
        init_sql_configmap  = optional(string)
      }))
  ...
  validation {
    condition     = contains(["8.0", "8.4", "9.0"], var.instance.spec.version_config.mysql_version)
    error_message = "MySQL version must be one of: 8.0, 8.4, 9.0."
  }
  ...
  # FIXED: Null-safe password validation - completely safe approach
  validation {
...
# SIMPLIFIED: Match the actual input structure from Kubernetes cluster
variable "inputs" {
  ...
  type = object({
    kubernetes_cluster = object({
      ...
      # REMOVED the complex providers object since it's causing the error
    })
  })
}