...
# Kubernetes Namespace for MongoDB Cluster
resource "kubernetes_namespace" "mongodb_cluster" {
...
module "mongodb_cluster" {
  ...
  release_name = "mongocluster-${local.cluster_name}-${substr(var.inputs.kubeblocks_operator.interfaces.output.release_id, 0, 8)}"

  depends_on = [
    kubernetes_namespace.mongodb_cluster
  ]
