variable "instance" {
  ...
  validation {
    condition     = !var.instance.spec.restore_config.restore_from_snapshot || var.instance.spec.restore_config.snapshot_arns != null
    error_message = "Snapshot ARN must be provided when restore_from_snapshot is true"
  }
}
