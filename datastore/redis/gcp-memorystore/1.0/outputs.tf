locals {
  # Essential Redis instance metadata and configuration
  output_attributes = {
  ...
  # Redis connection details for applications
  output_interfaces = {
    cluster = {
      ...
      # Use rediss:// scheme when TLS is enabled for automatic TLS configuration in clients
      ...
      secrets     = ["auth_token", "connection_string"] # Fields marked as sensitive
    }
  }
}